    ...
    switch( charPositionY ) {
        case 0:
            displayCodeWrite( DISPLAY_RS_INSTRUCTION,
                              ...
                              ( DISPLAY_20x4_LINE1_FIRST_CHARACTER_ADDRESS +
                                charPositionX ) );
            systick_delay_us(DISPLAY_DEL_37US); // <--- CAMBIO
        break;
...
static void displayPinWrite( uint8_t pinName, int value )
{
    // CORRECCIÓN: Si 'value' es distinto de LOW (0), entonces seteamos el pin en ALTO.
    GPIO_PinState state = (value != LOW) ? GPIO_PIN_SET : GPIO_PIN_RESET;
        ...
        case DISPLAY_CONNECTION_GPIO_4BITS:
            switch( pinName ) {
                ...
                case DISPLAY_PIN_D7:
                    ...
                    break;
                case DISPLAY_PIN_RS:
                    HAL_GPIO_WritePin(D8_GPIO_Port, D8_Pin, state);
                    break;
                case DISPLAY_PIN_EN:
                    HAL_GPIO_WritePin(D9_GPIO_Port, D9_Pin, state);
                    break;
                case DISPLAY_PIN_RW:
                    // RW a GND por hardware. No hacemos nada.
                    break;
                default: break;
            }
            break;
